...
double wheel1X=0,wheel1Y=0;
double wheel2X=60,wheel2Y=0;
...
vector< pair<double,double> > getVertices(double centerX,double centerY,double radius)
{
    vector< pair<double,double> > vertices;
    pair <double,double> vertex;
    for(int i=1;i<360;i++)
    {
        vertex=make_pair(centerX+radius*cos(i*(M_PI/180)),centerY+radius*sin(i*(M_PI/180)));
        vertices.push_back(vertex);
    }
    return vertices;
}

int checkCollision(double radius1,double radius2)
{
    double dist=sqrt((wheel1X-wheel2X)*(wheel1X-wheel2X)-(wheel1Y-wheel2Y)*(wheel1Y-wheel2Y));
...
void draw ()
{
    ...
    int collide=checkCollision(30,30);
    //Left Wheel
    if(collide==0)
    {
        for(int i=0;i<360;i++)
        ...
        for(int j=0;j<360;j++)
        {
            drawobject(wheel2,glm::vec3(wheel2X,wheel2Y,0),j,glm::vec3(0,0,1));
        }
    }
}
